...
from training.networks_stylegan2 import DiscriminatorBlock, MappingNetwork, DiscriminatorEpilogue,DiscriminatorEpilogue_eg3d,DiscriminatorUV
...
class DualDiscriminator(torch.nn.Module):
    def __init__(self,
        ...
        if c_dim > 0:
            self.mapping = MappingNetwork(z_dim=0, c_dim=c_dim, w_dim=cmap_dim, num_ws=None, w_avg_beta=None, **mapping_kwargs)
        self.register_buffer('transition', torch.ones([])) 

        self.b4 = DiscriminatorEpilogue_eg3d(channels_dict[4], cmap_dim=cmap_dim, resolution=4, **epilogue_kwargs, **common_kwargs)
        self.register_buffer('resample_filter', upfirdn2d.setup_filter([1,3,3,1]))
        self.disc_c_noise = disc_c_noise


        # common_kwargs = dict(img_channels=img_channels*3, architecture=architecture, conv_clamp=conv_clamp)
        # cur_layer_idx = 0
        # for res in self.block_resolutions:
        #     in_channels = channels_dict[res] if res < img_resolution else 0
        #     tmp_channels = channels_dict[res]
        ...
        #     block = DiscriminatorBlock(in_channels, tmp_channels, out_channels, resolution=res,
        #         first_layer_idx=cur_layer_idx, use_fp16=use_fp16, **block_kwargs, **common_kwargs)
        #     setattr(self, f'b{res}_2', block)
        #     cur_layer_idx += block.num_layers
        
        # self.b5 = DiscriminatorEpilogue_eg3d(channels_dict[4], cmap_dim=0, resolution=4, **epilogue_kwargs, **common_kwargs)


    def forward(self, img_input, c, update_emas=False, **block_kwargs):
        # import pdb; pdb.set_trace()
        # image_raw = filtered_resizing(img_input['image_raw'], size=img_input['image'].shape[-2], f=self.resample_filter,filter_mode='half')
        
        # image_raw = filtered_resizing(img_input['image_raw'], size=img_input['image'].shape[-2], f=self.resample_filter,filter_mode='half')
        
        # img = torch.cat([img_input['image'],image_raw ], 1)
        # import pdb; pdb.set_trace()
        img = img_input['image']
        ...
        # import pdb; pdb.set_trace()
        if self.c_dim > 0:
            if self.disc_c_noise > 0: c += torch.randn_like(c) * c.std(1) * self.disc_c_noise
            cmap = self.mapping(None, c)
        # x_image = x
        logits = self.b4(x, img, cmap)
       
        return logits
        # logits_c = torch.zeros_like(logits)
        img =  torch.cat([img_input['image'], image_raw ,img_input['mesh_normal']], 1)
        # img =  
        x = None
        for res in self.block_resolutions:
            block = getattr(self, f'b{res}_2')
            x, img = block(x, img, **block_kwargs)
        # # import pdb; pdb.set_trace()
        # x_uv = x
        # import pdb; pdb.set_trace()
        logits_c = self.b5(x, img, None)
...
@persistence.persistent_class
class DualDiscriminator_eg3d(torch.nn.Module):
    def __init__(self,
        c_dim,                          # Conditioning label (C) dimensionality.
        img_resolution,                 # Input resolution.
        img_channels,                   # Number of input color channels.
