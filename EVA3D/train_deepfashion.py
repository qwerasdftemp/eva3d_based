...
def train(opt, experiment_opt, _loader_dict, generator, discriminator, g_optim, d_optim, g_ema, device, augmentpipe):
    ...
    fid_num = 500
    fid_random_idx = [
        random.randint(0, len(_loader.dataset)-1) for _ in range(fid_num)
    ]
    fid_sample_z_list = [
        torch.randn(1, opt.style_dim, device=device) for _ in range(fid_num)
    ]
    ...
    for idx in pbar:
        ...
        if get_rank() == 0:
            ...
            if i % 10000 == 0 and i!=0:
                ...
                target_folder = os.path.join(opt.checkpoints_dir, experiment_opt.expname, 'volume_renderer/FID', f"fid_{str(i).zfill(7)}")
                if not os.path.exists(target_folder):
                    os.makedirs(target_folder)
                mean_latent = g_ema.mean_latent(10000, device)
                # import pdb; pdb.set_trace()
                for fid_i in tqdm(range(fid_num)):
                    num_viewdirs = 1
                    fid_sample_z = fid_sample_z_list[fid_i]
                    fid_sample_trans, fid_sample_beta, fid_sample_theta = _loader.dataset.sample_smpl_param(num_viewdirs, device, val=False, idx=fid_random_idx[fid_i])
                    fid_sample_cam_extrinsics, fid_sample_focals = _loader.dataset.get_camera_extrinsics(num_viewdirs, device, val=False)
                    out = g_ema(
                        [fid_sample_z],
                        fid_sample_cam_extrinsics,
                        fid_sample_focals,
                        fid_sample_beta,
                        fid_sample_theta,
                        fid_sample_trans,
                    ...
                    cur_rgb = out[1].detach().cpu().numpy()[..., :3]
                    cur_rgb = ((cur_rgb + 1) / 2) * 255
                    cur_rgb = Image.fromarray(cur_rgb.reshape(opt.renderer_output_size[0], opt.renderer_output_size[1], 3).astype(np.uint8))
                    cur_rgb.save(os.path.join(target_folder, f"{str(fid_i).zfill(6)}.png"))
                    g_ema.zero_grad()
                    del out
                #     fid_score = get_fid(target_folder, opt.fid_path)
            #     if fid_score < best_fid_score:
            #         best_fid_score = fid_score
            #         best_fid_iter = i
            #     print(f">>> Current FID: {fid_score}; Current Best FID: {best_fid_score}; Best FID Iter: {best_fid_iter} <<<")
