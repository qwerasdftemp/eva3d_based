    ...
    def forward(self, styles, cam_poses, focals, beta, theta, trans,
        ...
        if inv_Ks is None:
            if w_space:
                ...
                ff
                thumb_rgb, features, _sdf, mask, xyz, eikonal_term = self.renderer(cam_poses, focals, beta, theta, trans, styles=latent[0], truncation=truncation,\
                                                                                return_eikonal=return_eikonal,
                                                                                w_space=w_space, gamma_list=gamma_list, beta_list=beta_list,
                                                                                fix_viewdir=fix_viewdir)
            else:
                thumb_rgb, features, _sdf, mask, xyz, eikonal_term = self.renderer(cam_poses, focals, beta, theta, trans, styles=styles[0],truncation=truncation, \
