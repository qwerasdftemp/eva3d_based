...
class StyleGAN2Loss(Loss):
    def __init__(self, device, G, D ,augment_pipe=None,sdf_weight=1,residual_weight=1, r1_gamma=10, style_mixing_prob=0, pl_weight=0, pl_batch_shrink=2, pl_decay=0.01, pl_no_weight_grad=False, blur_init_sigma=0, blur_fade_kimg=0, r1_gamma_init=0, r1_gamma_fade_kimg=0, neural_rendering_resolution_initial=64, neural_rendering_resolution_final=None, neural_rendering_resolution_fade_kimg=0, gpc_reg_fade_kimg=1000, gpc_reg_prob=None, dual_discrimination=False, filter_mode='antialiased'):
        ...
        self.global_count=1e8*self.device.index

    def run_G(self, z, c, gen_flame, swapping_prob, neural_rendering_resolution, additional_dict={},update_emas=False):

        additional_dict = {'flame_para': gen_flame}
        ...
        gen_output = self.G.synthesis(ws, c, additional_dict=additional_dict,neural_rendering_resolution=neural_rendering_resolution, update_emas=update_emas)
        return gen_output, ws

    def run_D(self, img, c, flame_para ,blur_sigma=0, blur_sigma_raw=0, update_emas=False):
        ...
        logits = self.D(img, torch.cat([c,flame_para[:,100:]],dim=1), update_emas=update_emas)
        # logits = self.D(img, c, update_emas=update_emas)
        return logits 


    def run_D_base(self, img, c, blur_sigma=0, blur_sigma_raw=0, update_emas=False):
        blur_size = np.floor(blur_sigma * 3)
        if blur_size > 0:
            with torch.autograd.profiler.record_function('blur'):
                f = torch.arange(-blur_size, blur_size + 1, device=img['image'].device).div(blur_sigma).square().neg().exp2()
        ...
        logits = self.D_base(img, c, update_emas=update_emas)
        return logits    

    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c,real_flame, gen_flame,gain, cur_nimg,num_gpus=1):
        ...
        if phase in ['Gmain', 'Gboth']:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                gen_img, _gen_ws = self.run_G(gen_z, gen_c,gen_flame=gen_flame,swapping_prob=swapping_prob, neural_rendering_resolution=neural_rendering_resolution)
                # gen_logits = self.run_D(gen_img, gen_c, blur_sigma=blur_sigma)
                gen_logits = self.run_D(gen_img, gen_c,gen_flame, blur_sigma=blur_sigma)
                ...
                #print('Loss/scores/fake', gen_logits)
                #print('Loss/signs/fake', gen_logits.sign())
                #print('Loss/G/loss', loss_Gmain)

                pred_sdf = gen_img['pred_sdf']
                loss_pred_sdf = self.sdf_weight* torch.nn.functional.mse_loss(pred_sdf, torch.zeros_like(pred_sdf)).mean()
                # gen_logits_base = self.run_D_base(gen_img, gen_c, blur_sigma=blur_sigma)
                training_stats.report('Loss/scores/loss_pred_sdf', loss_pred_sdf)

                # residual = gen_img['residual']
                # loss_residual = self.residual_weight* torch.nn.functional.mse_loss(residual, torch.zeros_like(residual)).mean()
                # # gen_logits_base = self.run_D_base(gen_img, gen_c, blur_sigma=blur_sigma)
                # training_stats.report('Loss/scores/loss_residual', loss_residual)

                # training_stats.report('Loss/scores/fake_base', gen_logits_base)
                # training_stats.report('Loss/signs/fake_base', gen_logits_base.sign())
                # loss_Gmain_c_base = torch.nn.functional.softplus(-gen_logits_base)

                
                if num_gpus > 1:
                    loss_Gmain = loss_Gmain  + loss_pred_sdf
                else:
                    loss_Gmain = loss_Gmain  + loss_pred_sdf
        ...
        if phase in ['Greg', 'Gboth'] and self.G.rendering_kwargs.get('density_reg', 0) > 0 and self.G.rendering_kwargs['reg_type'] == 'l1':
            ...
            sigma = self.G.sample_mixed(all_coordinates, torch.randn_like(all_coordinates), ws, update_emas=False,all_flame_para=gen_flame)['sigma']
        ...
        if phase in ['Dmain', 'Dboth']:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                ...
                gen_img, _gen_ws = self.run_G(gen_z, gen_c,gen_flame=gen_flame, swapping_prob=swapping_prob, neural_rendering_resolution=neural_rendering_resolution, update_emas=True)
                gen_logits = self.run_D(gen_img, gen_c,gen_flame, blur_sigma=blur_sigma, update_emas=True)
        ...
        if phase in ['Dmain', 'Dreg', 'Dboth']:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                # b = ress
                # all_flame_para =  real_flame
       
                # shape = all_flame_para[:,0:10]
                # pose = all_flame_para[:,10:].view(-1,24,3,3)

                # self.flame_model = self.flame_model.to(shape.device)
                # vertices,canonical_vertices= self.flame_model(pose,shape)

                # cam2world_matrix = real_c[:, :16].view(-1, 4, 4)
                # intrinsics = real_c[:, 16:25].view(-1, 3, 3)

                # mesh_image= self.render_mesh(vertices,cam2world_matrix,intrinsics,real_img_tmp_image.shape[2])
                # real_img_tmp['mesh_normal'] = mesh_image

                # import PIL.Image
                # import os
                # mesh_image = real_img_tmp['mesh_normal'] 
                # img = (mesh_image.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8).cpu().detach().numpy()[0]
                # PIL.Image.fromarray(img, 'RGB').save(os.path.join('checkpoints/debug_images','%09d_real_normal.png'%self.global_count))


                # img = (real_img_tmp_image.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8).cpu().detach().numpy()[0]
                # PIL.Image.fromarray(img, 'RGB').save(os.path.join('checkpoints/debug_images','%9d_real_img.png'%self.global_count))
                # self.global_count+=1

                real_logits = self.run_D(real_img_tmp, real_c,real_flame, blur_sigma=blur_sigma)
                training_stats.report('Loss/scores/real', real_logits)
                training_stats.report('Loss/signs/real', real_logits.sign())

                #print('Loss/scores/real', real_logits)
                #print('Loss/signs/real', real_logits.sign())
                ...
                if phase in ['Dreg', 'Dboth']:
                    ...
                    #print('Loss/r1_penalty', r1_penalty)
                    #print('Loss/D/reg', loss_Dr1)
