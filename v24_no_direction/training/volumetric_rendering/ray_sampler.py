...
class RaySampler(torch.nn.Module):
    ...
    def forward(self, cam2world_matrix, intrinsics, resolution,square=False):
        """
        ...
        uv = torch.stack( torch.meshgrid(torch.arange(H, dtype=torch.float32, device=cam2world_matrix.device), torch.arange(W, dtype=torch.float32, device=cam2world_matrix.device),indexing='ij')) 
        uv[0] *= 1/H
        uv[1] *= 1/W
        # import pdb; pdb.set_trace()
