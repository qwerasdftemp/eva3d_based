...
class DemoDataset(Dataset):
    def __init__(self, sample_data_path='../data_for_run/assets/sample_data.pkl'):
...
class DeepFashionDataset(Dataset):
    def __init__(self, path, transform, resolution=(256, 128), nerf_resolution=(128, 64), flist=None,
        ...
        smpl_cfgs = {
            'model_folder': '../data_for_run/smpl_models',
            'model_type': 'smpl',
            'gender': 'neutral',
            'num_betas': 10
        }
...
class AISTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = np.array(_img.resize(self.resolution, Image.HAMMING))
        ratio = (img != 255).sum() / (img.shape[0] * img.shape[1] * img.shape[2])
        # if ratio > 0.2: # 0.5
            # print("Skipping...")
            # return self.__getitem__((index + 1) % self.__len__())

        thumb_img = np.array(_img.resize(self.nerf_resolution, Image.HAMMING))
        img = self.transform(img)
        thumb_img = self.transform(thumb_img)
...
class UBCFashionDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = np.array(cur_img.resize(self.resolution, Image.LANCZOS))
        thumb_img = np.array(cur_img.resize(self.nerf_resolution, Image.LANCZOS))
        img = self.transform(img)
        thumb_img = self.transform(thumb_img)
